...
export default function ProductDetailPage({ id }: { id: string }) {
    ...
    useEffect(() => {
        if (responseData?.status && responseData.data?.data) {
            ...
            let foundProduct = responseData.data.data.find((p: any) => p.id.toString() === id);
            // Jika tidak ketemu dengan ID yang pas, pakai index pertama sbg dummy
            if (!foundProduct && responseData.data.data.length > 0) {
                foundProduct = responseData.data.data[0];
            }
